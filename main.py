...
def main():
    ...
    for i in range(1, Ingest.get_number_of_groups() + 1):
        ...
        clearance_values: df = Ingest.get_clearance_values_for_group(df=df, group_chip_ids=chip_id_lst)
    ...
    for prot in clearance_protocols:
        grouped: pd.DataFrame.GroupedBy = Ingest.get_df_grouped_by_chip_section(prot.percent_cleared)
        data = Plotter.create_data_structure_for_plotting(grouped=grouped, y_column_name='percent_cleared')
